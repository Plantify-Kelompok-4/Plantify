        ...
        @if($location->lands && $location->lands->count() > 0)
            @foreach($location->lands as $land)
                <div class="list-group-item d-flex justify-content-between align-items-center" onclick="window.location='{{ route('lands.show', $land->id) }}'" style="cursor: pointer;">
                    ...
                    <div>
                        <button class="btn btn-warning btn-sm" onclick="event.stopPropagation(); showEditLandForm('{{ $land->id }}', '{{ $land->name }}', '{{ $land->area }}')">Edit</button>
                        <form action="{{ route('lands.destroy', $land->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger" onclick="return confirm('Apakah Anda yakin ingin menghapus lahan ini?')">Hapus</button>
                        </form>
                    </div>
                </div>
            @endforeach
...
<div id="editLandModal" class="modal" style="display:none;">
    <div class="modal-content" style="width: 300px; max-width: 90%; margin: auto; padding: 20px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2); position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);">
...
<script>
    ...
    function showEditLandForm(landId, landName, landArea) {
        document.getElementById('editLandId').value = landId;
        document.getElementById('editName').value = landName;
        document.getElementById('editArea').value = landArea;
        document.getElementById('editLandForm').action = '/lands/' + landId; // Set action ke rute update
    ...
    window.onclick = function(event) {
        var modal = document.getElementById('editLandModal');
